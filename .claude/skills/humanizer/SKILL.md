---
name: humanizer
description: Remove signs of AI-generated writing from prose the workspace itself authors: documentation, skill and command files, commit messages, and the human-facing prose in shortlist and rank output. Use when writing, editing or reviewing that text. Never applied to candidate facts, vacancy text or stored records, and never a licence to write application materials.
license: MIT
metadata:
  version: "2.9.1"
---
...
This is an AUTHORING skill. It governs prose THIS WORKSPACE WRITES; it governs nothing it reads, stores or decides.

- It applies to `README.md`, `CLAUDE.md`, skill and command files, code comments, commit messages, and the human-facing prose in `/rank` and `/shortlist` output.
- It never widens the product boundary. `discover -> verify -> match -> rank -> shortlist -> stop` is unchanged. Making prose sound human is not authorisation to write a cover letter, an outreach message, a tailored CV or any other application material; those remain outside this project regardless of who asks.
...
- NEVER APPLY IT TO EVIDENCE. Candidate facts, vacancy text, cached `description_text`, quoted blocker evidence and any stored record are data to be preserved exactly. A job advert that reads like AI slop is a job advert that reads like AI slop; rewriting one destroys the quotation a hard blocker depends on.
- Where it disagrees with `.claude/skills/job-matcher/writing-style.md`, that file wins for job-search output. It is the narrower authority: British English, and screening prose compact enough to decide from. The two agree on the rules that matter most here, including cutting em dashes and refusing invented claims.
- Where it disagrees with `CLAUDE.md`, the project rules win.

Rule 3's "Never invent facts" is the load-bearing one in this workspace and is stricter here than upstream: `CLAUDE.md` already forbids inventing experience, metrics, dates, salary, sponsorship or vacancy status, and no rewrite may reach for a specific number to replace a vague one.
...
MIT licensed. Derived from the upstream `humanizer` skill, itself based on Wikipedia's "Signs of AI writing" guide maintained by WikiProject AI Cleanup.

The numbered pattern sections are reproduced from the upstream skill with their wording unchanged, and the `description` was narrowed to this workspace's boundary. The `## Scope in this workspace` section is this workspace's own integration note and is not upstream content.

Unlike `karpathy-guidelines/SKILL.md`, this file KEEPS its non-ASCII characters rather than converting them to the workspace's ASCII convention. Sections 14, 18 and 19 are rules ABOUT the em dash, the emoji and the curly quote; a rule that says "scan the draft for `—`" cannot be written without one. This is a file read into context, never printed through the console, so it does not meet the cp1252 stdout hazard that `_force_utf8_stdout()` exists for.
