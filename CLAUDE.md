...
**Scores are decision support, not probabilities.** They predict nothing about interviews or offers. Bands are calibrated against the ceiling real adverts actually reach rather than against 100, which stays reachable in principle and almost never in practice, so every eligible Direct role from 54 to 65 stays visible for human review. A score alone can never create a hard blocker or a suppression record. REPORT THE BAND, NOT THE BARE NUMBER: the thresholds moved on 2026-09-03, so a reader calibrated on the old scale hears 66 as mediocre when it means Strong Match. A score reported to a person always carries its band name, and outside this workspace the band name travels alone. `Verify First` is an ACTION on a scored role, not a category.
...
- `.claude/skills/humanizer/` governs prose this workspace AUTHORS: documentation, instruction files, commit messages and human-facing output. It never touches evidence, because rewriting a cached advert destroys the quotation a blocker quotes, and sounding human is not authorisation to write application materials. Where it and `.claude/skills/job-matcher/writing-style.md` disagree, that file wins for job-search output.
